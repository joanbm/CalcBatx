 ...
 * Funcio cos - Cosinus.
 */
public class FuncioCos extends Funcio {
    public FuncioCos() {
        super("cos", 1);
    }
    ...
    public Complex calcular(Complex[] params) throws ExpressioException {
        if (params[0].esComplex())
            throw new ExpressioException("Encara no es pot operar amb nombres complexos amb cos.");

        return new Complex(
            Math.cos(params[0].re()),
            0.0
        );
    }
    ...
    @Override public Expressio derivada(Expressio[] params) throws ExpressioException {
        return new ExpressioFuncio("multiplicar", new Expressio[] {
            new ExpressioFuncio("negar", new Expressio[] {
                new ExpressioFuncio("sin", new Expressio[] {
                    params[0]
                })
            }),
            params[0].derivada()
