...
/**
 * Funcio arcsin - Arc sinus.
 */
public class FuncioArcSin extends Funcio {
    public FuncioArcSin() {
        super("arcsin", 1);
    }
    ...
    public Complex calcular(Complex[] params) throws ExpressioException {
        if (params[0].esComplex())
            throw new ExpressioException("Encara no es pot operar amb nombres complexos amb arcsin.");

        return new Complex(
            Math.asin(params[0].re()),
            0.0
        );
    }
